...
const ANALYSIS_KNOWLEDGE_ENTRIES: AnalysisKnowledgeEntry[] = [
  {
    ...
    content:
      "The final project methodology uses 100 m grids for computation, H3 resolution 9 for WebGIS visualization, and municipality as a dominant attribute. Core formulas include PTD = 1 - PTA, ESD = 1 - ESA, EOTD = M * (0.40*SI + 0.25*DS + 0.20*GP + 0.15*DI), TPHS = min(100, 100*(0.40*SVI + 0.30*PTD + 0.20*ESD + 0.10*EOTD)*(1 + 0.20*min(SVI, PTD, ESD))), IPI = min(100, 100*(0.45*TPHS_norm + 0.20*VPE + 0.15*ESC + 0.10*FEAS + 0.10*GM)*CA), DCS = 100*(0.30*source_completeness + 0.25*spatial_resolution + 0.20*temporal_freshness + 0.25*data_directness), and CA = 0.85 + 0.15*DCS/100.",
  },
  {
    ...
    content:
      "The TP-IPT decision layer contains 15,547 H3 resolution 9 cells, 168 hotspot clusters, and 5,970 high-priority H3 cells. Mean scores are SVI 36.06, PT deficit 65.521, essential services deficit 70.524, EO territorial disadvantage 33.337, hotspot score 55.134, intervention priority score 55.748, and data confidence score 87.277. Priority classes are distributed as Low priority 21.78%, Medium priority 20.87%, High priority 20.36%, Monitor 18.95%, and Strategic priority 18.04%. The dominant typology is General transport poverty hotspot at 65.46% of cells.",
  },
  {
  ...
  },
  {
    ...
    title: "Social vulnerability evidence and caveats",
  ...
  },
  {
    ...
    questions: [
      "What does EO territorial disadvantage mean?",
  ...
  },
  {
    ...
    content:
      "Intervention sequencing should start from the dominant drivers and confidence level. Public transport deficit points to frequency, stop access, redundancy, feeder access, and service coverage validation before new infrastructure. Essential services deficit points to checking service POIs, opening hours, catchments, capacity, and walking-network access. Social vulnerability requires checking whether affected groups are genuinely exposed to the transport or service gap. EO territorial disadvantage should be used as context for growth, built-up intensity, isolation, or land-use mismatch rather than as a standalone prescription.",
  },
  {
  ...
  },
  {
    ...
    content:
      "Project typologies are rule-based and include General transport poverty hotspot, Essential-services desert, Active but disconnected area, Dense underserved fringe, Dispersed rural fragility, Growth-transport mismatch, Lower concern, and related monitoring categories. When multiple rules match, the strongest matching signal is used. Typology should guide the explanation and initial intervention family but should not replace the underlying score breakdown and local validation.",
  },
];
